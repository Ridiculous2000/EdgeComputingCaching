...
# 初始参数
Ahat = np.random.rand(2, 2)  # Ahat的初始估计值，形状为(2, 2)
mhat = np.random.rand(2)  # mhat的初始估计值，形状为(2,)
omega = 0.5  # omega的初始值

# 输入数据
seq = np.array([[1.0, 0], [2.0, 1], [3.0, 0], [4.0, 1], [5.0, 0], [6.0, 1]])
# seq是一个观测序列，每一行包含事件的时间和对应的节点ID。
# 在这个样例中，有两个节点，节点ID为0和1，事件的时间从1.0到6.0

Ahat_estimate, mhat_estimate = MHP.EM(Ahat, mhat, omega, seq)
